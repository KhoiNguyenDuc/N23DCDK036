...
struct Speaker {
    string ten;              // Tên diễn giả
    string soDienThoai;     // Số điện thoại
    string chuDe;           // Chủ đề bài phát biểu
    float phiDienThuyet;    // Phí diễn thuyết
};
...
void timKiemDienGiaTheoChuDe(Speaker speakers[], int soLuong, string tuKhoa) {
    bool timThay = false;
    
    cout << "\nDanh sach dien gia co chu de khop voi: " << tuKhoa << "\n";
    for (int i = 0; i < soLuong; i++) {
        if (speakers[i].chuDe.find(tuKhoa) != string::npos) {  // Tìm từ khóa trong chủ đề
            timThay = true;
            cout << "\nDien gia " << i + 1 << ":\n";
            cout << "Ten: " << speakers[i].ten << endl;
            cout << "So dien thoai: " << speakers[i].soDienThoai << endl;
    ...
    if (!timThay) {
        cout << "\nKhong tim thay dien gia nao co chu de khop voi tu khoa '" << tuKhoa << "'.\n";
    }
}
    ...
    do {
        ...
        switch (choice) {
            ...
            case 4: {
                cin.ignore();  // Để bỏ qua ký tự '\n' sau khi đọc số
                string tuKhoa;
                cout << "Nhap tu khoa chu de: ";
                getline(cin, tuKhoa);
                timKiemDienGiaTheoChuDe(speakers, soLuong, tuKhoa);
                break;
            }
            case 5:
...
int main() {
    Speaker speakers[10];  
    int soLuong = 0;        // Biến để lưu số lượng diễn giả

    menu(speakers, soLuong);  // Gọi hàm menu
